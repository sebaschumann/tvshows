class SeasonsController < ApplicationController
  ...
  def index
    @seasons = Season.all
    @seasons = @seasons.joins(:episode).where(:episode =>
       {:id => params[:episode_number]}) if params[:episode_number]
        @seasons = @seasons.where(["number LIKE :filter", :filter =>
  ...
  private
    ...
    def season_params
      params.require(:season).permit(:number, :begins_at, :novela_id)
    end
end
